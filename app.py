...
def extract_imports_from_file(file_path):
    ...
    standard_libs = {'os', 'sys', 'time', 'json', 're', 'math', 'random', 
                     ...
                     'ast', 'abc', 'array', 'asynchat', 'asyncio', 'asyncore',
                     'atexit', 'audioop', 'binascii', 'binhex', 'bisect',
                     'builtins', 'bz2', 'calendar', 'cgi', 'cgitb', 'chunk',
                     'cmath', 'cmd', 'code', 'codecs', 'codeop', 'colorsys',
                     'compileall', 'concurrent', 'configparser', 'contextlib',
                     'contextvars', 'copy', 'copyreg', 'cProfile', 'csv',
                     'ctypes', 'curses', 'dataclasses', 'decimal', 'difflib',
                     ...
                     'gzip', 'heapq', 'hmac', 'html', 'http', 'idlelib',
                     'imaplib', 'imghdr', 'imp', 'importlib', 'inspect',
                     'ipaddress', 'json', 'keyword', 'lib2to3', 'linecache',
                     'locale', 'lzma', 'mailbox', 'mailcap', 'marshal',
...
def install_libraries(venv_path, libraries, task_id):
    ...
    for i, lib in enumerate(libraries, 1):
        ...
        try:
            # تحويل اسم المكتبة إذا كان موجود في القاموس
            actual_lib = LIBRARY_MAPPING.get(lib, lib)
            
            # محاولة تثبيت المكتبة
            result = subprocess.run([pip_path, 'install', actual_lib, '--no-cache-dir'],
            ...
            if result.returncode == 0:
                ...
                # محاولة تثبيت باسم مختلف
                alt_name = lib.replace('.py', '').replace('_', '-')
                try:
